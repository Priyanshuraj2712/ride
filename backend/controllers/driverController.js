...
if (!vehicle) {
vehicle = await VehicleModel.create({ driver: driver._id, vehicleNumber, totalSeats, model, color });
...
const drivers = await DriverModel.find({ online: true, seatsAvailable: { $gt: 0 }, liveLocation: {
  $nearSphere: {
    $geometry: {
      type: 'Point',
...
}).limit(10).populate('user vehicle');
res.json({ drivers });
} catch (err) { next(err); }
};
